...
## v118 · 2026-09-26 · `5b94721` · fix

**负责人群播报三问题修复（曼波 9-25 记录：有负责人报成无负责人/当日逾期漏播/无子项目报「子项目均完成」，全部以生产表实据定位）**

- 提交说明：fix: 负责人群播报三问题——整合卡 @ 全部责任人、占位支持行不计入僵尸子项目
- **问题1+2 同根**（有负责人报「未指派」/负责人「没被播报」）：`buildLeaderDDLReportCard` 渲染硬编码 `mentionField='owner'`——负责人填在组字段（sjcontributers 等）、owner 为空的项目，整合卡一律显示「👤 未指派」且负责人收不到 @。实测 yolo魔改学习（owner 空/sj=贺韵洁）即此；冯仁**瀚**的项目负责人也全在 sj 列，他本人在负责人群卡里从未被 @，从负责人视角即「我的项目没播报」（问题 2 的观感；数据层实测 leader('all') 合格集 ⊇ 各群，不存在整行丢失）。修：`getFieldMembers` 新增 `'all'`——owner+四个组字段并集（按 id 去重，effMembers 归并同样生效），整合卡两栏改用 'all' 渲染；实测 yolo 行修复后为 `@贺韵洁 步兵组 - yolo魔改学习 - 今天到期`。
- **问题3**（无子项目却报「🧟 子项目均已收尾，父项目待结」）：实据=生产表四个被标 🧟 的项目都有已完成子行，其中自定义客户端/大符的子行是「（重装支持项目）/（基建支持支持项目）」类**占位支持行**。修（曼波口径）：名字整体括号包住的占位行（isPlaceholderChildName）不计入僵尸子项目判定——全占位子行的父项目按无子项目叶子行正常播报不打 🧟；真实子行全收尾的（实物demo/视觉导航）保留 9-17 口径继续打标。实测修复后 🧟 仅剩实物demo、视觉导航。
- 测试：stub-test-ddl-leader 新增 6 断言（组字段负责人 @/未指派消除/真无主行回退保留/真实僵尸保留/占位僵尸消除），35/35；九套全绿。
- 部署状态：与 v115-v117 同批待上线（负责人群三问题的修复要 `npm run push` 后才在生产生效）。

## v119 · 2026-09-26 · 随本提交落地 · fix

**团队负载宣运域组别系数再降：×0.5 → ×0.25（曼波拍板）**

- 提交说明：fix: 团队负载宣运域组别系数再调（宣经/宣运/宣运组 0.5→0.25）
- `server/src/services/workloadService.js` GROUP_COEFF 宣运域三键（项目 category「宣经」/「宣运」、工单面向组别「宣运组」）0.5 → 0.25——2026-09-24 拍板 ×0.5 的基础上再收紧「宣传运营类任务密度高但单项压力轻」的折减；重装/步兵/哨兵 ×1.2 不变；`weights.groupCoeff` 为引用透出自动跟随。
