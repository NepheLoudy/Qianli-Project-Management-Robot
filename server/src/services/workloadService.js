...
/**
 * 纯计算核心（不 IO，stub 测试直测）：项目列表 + ticket-bot 按人明细 → 负载全景。
 *
 ...
 */
function computeWorkload(projects, ticketData, nowMs) {
  const now = dayjs(nowMs);
  const persons = new Map(); // openId → { name, groupSet, score, projects, tickets }

  const ensurePerson = (openId, name) => {
    if (!persons.has(openId)) {
      persons.set(openId, { name: name || '未知', groupSet: new Set(), score: 0, projects: [], tickets: [] });
    }
  ...
  for (const p of projects) {
    ...
    for (const [openId, m] of memberMap) {
      ...
      const score = round2(statusW * timeW * priorityW * roleW);
  ...
  if (ticketData) {
    for (const [openId, tp] of Object.entries(ticketData.persons || {})) {
      ...
      for (const t of tp.tickets || []) {
        ...
        const score = round2((bucketW * timeW) / (t.shareCount || 1)); // 多人协作单按人数摊薄
        person.score = round2(person.score + score);
        person.tickets.push({
          code: t.code || '',
          title: t.title,
...
/**
 * 团队负载全景：项目表全量 + ticket-bot 按人未结单，聚合评分。
 * ticket-bot 不可用时降级为仅项目侧（ticketsSource='unavailable'），不 500。
 */
async function getTeamWorkload() {
  ...
  const result = computeWorkload(projects, ticketData, nowMs);
  return { generatedAt: new Date(nowMs).toISOString(), ticketsSource, ...result };
}
