...
- 支持指令系统（2026-09-14 起 `/help` 只向群聊展示成员相关指令，运维/诊断类仍可直接使用但不展示）：
  ...
  - `/status` — 查看服务运行状态 + 舰队健康快照（部署目标本机 7 服务 health 探测 + 网关长连接/投递统计，2026-09-21 v105 起——管理员私聊即可远程看状态，不依赖开发内网）（运维，不在 /help 展示；群内任何成员可触发为既定设计 2026-09-14 口径，非漏洞）
  - `/test-ddl` — 手动触发 DDL 播报测试（与正式播报同内容，可作部分失败后的补发）（运维，不在 /help 展示；群内任何成员可触发为既定设计 2026-09-14 口径，非漏洞）
...
- ⏳ **等回执待结单**：回执结单节点上无负责人 / 未填理想结单时间 / 超 7 日窗口的其余工单（2026-09-17 新增；等回执=没做完，不允许漏播——无负责人按面向组别兜底分组）
...
| `/api/hub/workload` | GET | 团队负载全景（工单+项目+被@三源评分聚合，2026-09-24 上线/同日算法升级；本地运维台「团队负载」看板数据源）。聚合本项目项目表全量（effMembers 父负责人归并）+ ticket-bot `/api/tickets/workload-by-person`（10s 超时，失败降级仅项目侧并标 `ticketsSource:'unavailable'`）+ 网关 `/api/usage/mentions`（近 7 天被@，5s 超时，失败缺该维度并标 `mentionsSource:'unavailable'`）。评分=状态折减(in_progress 1.0/waiting 0.6/pending 0.3)×DDL 时效(消耗比分档,逾期递增封顶 2.7)×重要性(项目 priority/工单分桶代理)×组别系数(宣运×0.25、重装/步兵/哨兵×1.2,按任务归属组别)×角色(owner 1.3)+被@接量(每被@一次+0.01 分,近 7 天自然日滑窗,零任务纯被@的协调角色也入榜)，多人单按 shareCount 摊薄；unclaimed 不计个人分、归组别待接。权重随 `weights` 字段透出可核对 |
